...
class ProductsController extends Controller
{
    ...
    public function viewOrders()
    {
        ...
        return view('admin.orders.view_orders')->with(compact('orders', 'allFlorists'));
    }
    ...
    function loadView(Request $request)
    {
        ...
        if ($jsonArrayResponse != '' && $jsonArrayResponse != null && $jsonArrayResponse->status == "Captured") {
            $paymentId = $jsonArrayResponse->token;
            $lastId = Session::get('lastOrderId');
            $ord_data = Orders::where('id', $lastId)->update(['showorder' => 1]);
            // mail error solving code starts here
            $ebloomfee = $amount * 15 / 100;
            $floatebloomfee = sprintf("%.2f", $ebloomfee);
            $final = explode('.', (string)$floatebloomfee);
            if ((int)$final[1] > 0) {
                $ebloomfinal = (int)$final[0] + 1;
            } else {
                $ebloomfinal = (int)$final[0];
            }
            $order = Orders::where('id', $lastId)->first();
            $florist = Florist::where('id', $order->florist_id)->first();
            $admin = admin::first();
            $products = Products::get();
            Mail::to($order->senderEmail ?? 'user@gmail.com')->send(new InProcessOrder($florist, $order, $products));
            ...
            // mail order solving code ends here
            $seller_id = $florist->seller_id;
            if($seller_id != '' && $seller_id != null){
                $curlPTSession = "https://api.everypay.gr/payments/{$paymentId}/transfers";
                $postPTrequest = array(
                    'seller' => $seller_id,
                    'amount' => $amount,
                    'commission_amount' => round($ebloomfinal),
                    'description' => 'Amount splited',
                );
            $curlPTSession = curl_init($curlPTSession);
            curl_setopt($curlPTSession, CURLOPT_USERPWD, $pk);
            curl_setopt($curlPTSession, CURLOPT_POSTFIELDS, $postPTrequest);
            curl_setopt($curlPTSession, CURLOPT_RETURNTRANSFER, true);
            $responsePT = curl_exec($curlPTSession);
            curl_close($curlPTSession);
            }
            // $curlPTSession = "https://sandbox-api.everypay.gr/payments/{$paymentId}/transfers";
