                                  ...
                                  success: function(data) {
                                    console.log(data);
                                      if ($.isEmptyObject(data.error)) {
                                          // alert(data.success);
                                          window.location.href = "{{ url(app()->getLocale().'/match_store')}}";
            ...
            <div class="row">
              ...
              <div class="shop-listing-content col-12 col-lg-9">
                ...
                <div class="shops-listing-shops position-relative mt-5">
                  ...
                  <input type="hidden" name="previousCount" id="previousCount">
                    <script>
                      ...
                      $(document).ready(function() {
                        // alert('check');
                        var florists = {!! json_encode($allFlorists->toArray()) !!};
                        var timetable = {!! json_encode($timetable->toArray()) !!};
                        var lang = {!! json_encode(app()->getLocale()) !!}
                        console.log('language: '+lang);
                        var userDate = {!! json_encode(Session::get('address')['date']) !!}
                        console.log("user date: "+userDate);

                        // get current day
                    var userSelectedDay = {!! json_encode(Session::get('address')['dayName']) !!}
                    ...
                    console.log('current day: '+userSelectedDay);
                    var currentDayName = '';
                    ...
                    var dt = new Date();
                    var currentHours = parseInt(dt.getHours());
                    var addTwoHours = currentHours+2;

                    console.log('hourTwoHours :'+addTwoHours);
                    ...
                    var currentday = dt.getDate();
                      console.log("current date: "+today);
                        ...
                        // florists = sortArray(florists);
                        // console.log("florists");
                        // console.log(florists);

                        florists.sort(function (a, b) {
                          return a.minimam_order - b.minimam_order;
                        });
                        // array[j].minimam_order
                        ...
                        florists.forEach(async florist => {
                          console.log('single florist');
                          console.log(florist);

                          var userAddress = {!! json_encode(Session::get('address')['completeAddress']) !!};
                          var storeAddress = florist.address;
                          ...
                          directionsService.route(request, function(response, status) {
                              ...
                              console.log('staus of florist: '+florist.name);
                              ...
                              if ( status == google.maps.DirectionsStatus.OK ) {
                                ...
                                var distaceInKm = distanceInMeter/1000;
                                    ...
                                    // $('.matched-florists').removeClass('d-none');
                                    
                                    
                                // console.log(florist.name +" distance: "+ distaceInKm);
                                console.log("before condition delivery limit: "+florist.delivery_limit);
                                console.log("florist: "+florist.name);
                                console.log("calculated distance: "+distaceInKm);
                                
                                if (distaceInKm <= florist.delivery_limit) {
                                  if (florist.status == 1) {
                                    if (florist.admin === 2) {
                                      console.log('florist admin: '+florist.admin);

                                      console.log('above condition to day check');
                                      var timeCount = 0;
                                      ...
                                      timetable.forEach(time => {
                                        ...
                                        if (time.status == '1' && time.day == currentDayName && time.florist_id == florist.id ) {
                                          console.log('time table pass: '+time.day);
                                          if (userDate == today) {
                                            if ( time.fromHour >= addTwoHours ) {
                                              ...
                                              count = count + 1;
                                              floristCount = floristCount + 1;

                                              // setTimeout(function () {
                                                var previousCount = $("#previousCount").val(); 
                                                if (floristCount == previousCount  ) {
                                                  // $('.searchWait').addClass('d-none');
                                                  // $('.matched-florists').removeClass('d-none');
                                                  // console.log('match previous');
                                                }else{
                                                  // $('.searchWait').removeClass('d-none');
                                                  // $('.matched-florists').addClass('d-none');
                                                  console.log('not match previous');
                                                  $("#previousCount").val(floristCount);
                                                ...
                                                >
                                                  ...
                                                  >
                                                    ...
                                                    >
                                                      ...
                                                      <img src="/uploads/products/${florist.back_image ?? '58630.jpeg' }" alt="florist image" class="shops-listing-shop-cover w-100 h-100">
                                        ...
                                        } //end timetable condition
                                      
                                      }); // end of timetable foreach
                                      
                                    }// check florist admin
                                  }//end order status condition
                                  // return false;
                              ...
                              $('.matched-florists').html(html);
                              if (floristCount === 1) {
                                $('.match-florist').html(`{{__('words.Βρήκαμε')}} (${floristCount}) {{__('words.ανθοπωλείο')}}  

                                `);
                                
                              }else if(floristCount >= 2) {
                                $('.match-florist').html(`{{__('words.Βρήκαμε')}} (${floristCount}) {{__('words.ανθοπωλεία')}} 
                                
                                `);
                              }
                              if (floristCount === 0 ) {
                                $('.match-florist').html(`{{__('words.Ουπς!Δεν μπορέσαμε να βρούμε ανθοπωλεία στη περιοχή σου.')}}`);
                              }
                              // $("#searchLoading").css("display","none");

                            });
                            // end of get distance section
                          function m_get_directions_route (request) {
                            directionsService.route(request, function(response, status) {
                              ...
                              if (floristCount === 0 ) {
                                ...
                                // $('.match-florist').html(`Ουπς!Δεν μπορέσαμε να βρούμε ανθοπωλεία στη περιοχή σου.`);
                              }
                            });
                            // end of get distance section
                          }
                        ...
                        setInterval(function(){ 
                          // alert(floristCount); 
                          var preCount = $("#previousCount").val(); 
                          // if (floristCount == preCount ) {
                          //   $('.searchWait').addClass('d-none');
                          //   $('.matched-florists').removeClass('d-none');
                          //   console.log(' match previous');
                          // }else{
