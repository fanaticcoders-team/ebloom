...
<section>
  ...
  <ul class="nav nav-pills flex-nowrap py-7">
    ...
    <li class="nav-item">
      <a class="nav-link text-white active" href="{{url(app()->getLocale().'/user/orders')}}">
  ...
  <div class="container">
    <div class="position-relative" style="margin-top: -25px; z-index: 1;">
        <div class="bg-white box-shadow rounded mb-5 mb-md-7">
           <div class="px-7 pt-7 pt-lg-11 px-lg-5 pt-xl-11 px-xl-9 mb-11">
               <h2>
                {{__('words.Οι Παραγγελίες μου')}}
                {{-- Your orders --}}
                </h2>

                                       {{-- <p class="text-muted">Are you in the mood for a coffee? Pizza? Burger? Whatever you're in the mood for, we have it!</p> --}}
           </div>
           @if (count($orders)>0)
                 ...
                 <table class="table table-striped table-hover">
                     ...
                     <tbody>
                       @foreach ($orders as $order)
                        @if ( $order->showOrder == 1)
                            
                            <tr>
                                <td>{{$order->orderNo}}</td>
                                {{-- <img src="/examples/images/avatar/1.jpg" class="avatar" alt="Avatar"> --}}
                                <td><a href="#"> {{Auth::user()->name}}</a></td>
                                <td>{{$order->address}}</td>
                                <td>{{$order->created_at}}</td>                        
                                {{-- <td><span class="status text-success">&bull;</span> Delivered</td> --}}
                                @if ($order->order_status == "New" || $order->order_status == "In Process")
                                ...
                                <td><a href="{{url(app()->getLocale().'/order/'.$order->id)}}" class="view" title="{{__('words.Δείτε λεπτομέρειες')}}" data-toggle="tooltip"><i class="material-icons"> &#xE5C8; </i></a></td>
                            </tr>
                        @endif
                       @endforeach
                         {{-- <tr>
           ...
           @else 
            <div class="px-7 px-lg-5 text-center pb-12">
              <img class="mb-5" src="{{asset("frontEnd-assets/site-assets/img/icons/bag.png")}}" alt="No orders">
              {{-- <span class="fa fa-gift"></span> --}}
              <h3 class="h1">
                Δεν υπάρχουν Παραγγελίες
                {{-- There are no orders --}}
                </h3>
              <p class="col-md-6 mx-auto text-muted mb-11">
                Εδώ, μπορείτε να δείτε τις παραγγελίες σας, να τις ελέγξετε και να τις παραγγείλετε ξανά με ένα μόνο κλικ.
                {{-- Here, you can see your orders, review them and order them again with just one click. --}}
                </p>
              <a href="{{url(app()->getLocale().'/welcome')}}" class="btn btn-primary btn-lg">
                Παραγγελία
                {{-- Begin your order --}}
                </a>
            </div>
    ...
    {{-- start no order section --}}
      
    {{-- end no order section --}}

</section>
{{-- end body section --}}
<script type="text/javascript">
    $(document).ready(function () {
        $(document).on('click', '.rate-restaurant', function () {
            var data = $(this).data();
            app.popup.openRateRestaurant(data.order_id, data.shop_id, false, data.hasOwnProperty('is_own_delivery'), $(this));
            return false;
        });
        $(document).on('click', '.add-past-order', function (e) {
            var d = $(this).data();
            app.reorder(d.order_id, d.shop_id, d.deliverytype, d.address_id);
            e.preventDefault();
            return false;
        });
    });
